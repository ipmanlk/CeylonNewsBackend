...
func TestDivainaScraper(t *testing.T) {
	// Create fetcher with HTTP client only for testing
	fetcher := createTestFetcher()

	scraper := NewDivainaScraper(fetcher)

	// Test basic properties
	if scraper.Name() != "Divaina" {
		t.Errorf("Expected name 'Divaina', got '%s'", scraper.Name())
	}
	...
	if languages[0] != model.LangSi {
		t.Errorf("Expected language 'si', got '%s'", languages[0])
	}
	...
	t.Run("Scrape Sinhala", func(t *testing.T) {
		...
		article := articles[0]
		if article.SourceName != "Divaina" {
			t.Errorf("Expected source name 'Divaina', got '%s'", article.SourceName)
		}
