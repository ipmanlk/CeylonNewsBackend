...
func TestGaganaScraper(t *testing.T) {
	...
	if scraper.Name() != "Gagana" {
		t.Errorf("Expected name 'Gagana', got '%s'", scraper.Name())
	}
	...
	t.Run("Scrape Sinhala", func(t *testing.T) {
		...
		article := articles[0]
		if article.SourceName != "Gagana" {
			t.Errorf("Expected source name 'Gagana', got '%s'", article.SourceName)
		}
