...
func TestLankapuvathScraper(t *testing.T) {
	// Create fetcher with HTTP client only for testing
	fetcher := createTestFetcher()

	scraper := NewLankapuvathScraper(fetcher)

	// Test basic properties
	if scraper.Name() != "Lankapuvath" {
		t.Errorf("Expected name 'Lankapuvath', got '%s'", scraper.Name())
	}
	...
	t.Run("Scrape English", func(t *testing.T) {
		...
		article := articles[0]
		if article.SourceName != "Lankapuvath" {
			t.Errorf("Expected source name 'Lankapuvath', got '%s'", article.SourceName)
		}
