...
func TestNethnewsScraper(t *testing.T) {
	// Create fetcher with HTTP client only for testing
	fetcher := createTestFetcher()

	scraper := NewNethnewsScraper(fetcher)

	// Test basic properties
	if scraper.Name() != "Neth News" {
		t.Errorf("Expected name 'Neth News', got '%s'", scraper.Name())
	}
	...
	t.Run("Scrape Sinhala", func(t *testing.T) {
		...
		article := articles[0]
		if article.SourceName != "Neth News" {
			t.Errorf("Expected source name 'Neth News', got '%s'", article.SourceName)
		}
